...
// ShowTimeModel represents a mgo database session with a showtime data model
type ShowTimeModel struct {
	C *mongo.Collection
}

// All method will be used to get all records from showtimes collection
func (m *ShowTimeModel) All() ([]models.ShowTime, error) {
	...
	// Find all showtimes
	showtimeCurser, err := m.C.Find(ctx, bson.M{})
	if err != nil {
		return nil, err
	}
	err = showtimeCurser.All(ctx, &st)
	if err != nil {
		return nil, err
	}
